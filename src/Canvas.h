...
class Canvas {
 public:
  ...
  uint16_t width()const{return w;} uint16_t height()const{return h;}
 private:
  void character(int x,int y,char c,uint8_t scale);
  uint8_t* data; uint16_t physicalW,physicalH,w,h,stride; Rotation orientation;
};
